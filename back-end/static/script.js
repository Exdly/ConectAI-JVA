document.addEventListener("DOMContentLoaded", () => {
    ...
    const els = {
      ...
      notif: $("notification"),
      status: document.querySelector(".status-indicator") // New
    }
    ...
    const checkAuthStatus = async () => {
        try {
            ...
            if (!state.auth) {
                // Fetch Auth URL explicitly if not authenticated
                fetchAuthUrl();
            }
        } catch (e) {
            console.error("[Chatbot] Error checking auth status:", e);
        }
    };

    const fetchAuthUrl = async () => {
        try {
            const res = await fetch(`${CFG.BACKEND}/api/auth/url`);
            const data = await res.json();
            if (data.auth_url) {
                // Mostrar banner persistente o mensaje inicial
                showAuthBanner(data.auth_url);
            }
        } catch (e) {
    ...
    const showAuthBanner = (url) => {
        // Remover banners previos si existen
        const existing = els.msgs.querySelector(".auth-banner");
        if (existing) existing.remove();

        const banner = h("div", { className: "message bot auth-banner" },
            h("div", { className: "message-bubble", style: { border: "2px solid #f39c12", background: "#fff3cd" } },
                h("div", { style: { fontWeight: "bold", marginBottom: "5px", color: "#856404" } }, "⚠️ Requiere Autorización"),
                h("div", {}, "Para acceder a la información, necesito permiso para leer los documentos de Google Drive."),
                h("a", { 
                    href: url, 
                    target: "_blank", 
                    className: "auth-btn",
                ...
                }, "Autorizar Ahora Google")
            )
        );
        els.msgs.prepend(banner); // Poner al principio
    };

    const updateAuthUI = (isAuth) => {
        if (els.status) {
            els.status.style.background = isAuth ? "#2ecc71" : "#e74c3c";
            els.status.title = isAuth ? "Conectado" : "Desconectado";
        }
    };
    ...
    const addLinkMessage = (text, linkTxt, url) => {
        // Evitar duplicados si ya hay un banner o mensaje similar
    ...
    const processMessage = async (msg) => {
        ...
        try {
          ...
          if (res.status === 401 || data.error === "not_authenticated") {
            ...
            if (authRes.auth_url) {
                showAuthBanner(authRes.auth_url)
                renderMessage("Por favor, autoriza el acceso a Google Drive usando el botón de arriba.", false)
            }
            else renderMessage("Error de autenticación. No se pudo obtener la URL.", false)
            ...
            // Update auth status to true if success
            state.auth = true
            updateAuthUI(true)
          } else {
          ...
          if (e.name === "AbortError") {
            ...
            renderMessage("Error de conexión con el servidor.", false)
          }
        } finally {
          hideTyping()
